...
public class TileEntityVoidicAlchemy extends TileEntityVoidicPowerInventory {
	...
	protected void onUpdate() {
		...
		if (!world.isRemote) {
			if (cooking) {
				cookingTick++;
				if (cookingTick % 10 == 0) ParticleFluffPacketHandler.spawnOnServer(world, new Vec3d(pos.getX() + 0.45F + (world.rand.nextFloat() * 0.10), pos.getY() + 0.5F, pos.getZ() + 0.45F + (world.rand.nextFloat() * 0.10)), new Vec3d(0, 0, 0), world.rand.nextInt(20) + 30, -((world.rand.nextFloat() * 0.02F) + 0.01F), (world.rand.nextFloat() * 0.75F) + 0.25F, (world.rand.nextInt(0xFFFFFF) << 8) + 0xFF);
