...
public class EntityVoidChain extends EntityArrow implements IProjectile, IEntityAdditionalSpawnData {
	...
	public EntityVoidChain(World worldIn) {
		super(worldIn);
		this.xTile = -1;
		this.yTile = -1;
		this.zTile = -1;
	...
	public EntityVoidChain(World worldIn, double x, double y, double z) {
		this(worldIn);
		this.setPosition(x, y, z);
	}

	public EntityVoidChain(World worldIn, EntityLivingBase shooter, EntityLivingBase target, float dmg) {
		...
		shoot(d0, d1 + d3 * 0.20000000298023224D, d2, 1.6F, (float) (14 - this.world.getDifficulty().getDifficultyId() * 4));
	}
